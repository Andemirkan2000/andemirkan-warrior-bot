...
# === НАСТРОЙКИ ===
TELEGRAM_TOKEN = os.getenv("7437647284:AAGpLBtbTirmLZjVK1qITyIT2fzhBggEjs4")
...
def main():
    ...
    scheduler.add_job(send_reminders, 'cron', hour=9, minute=0, args=[updater.bot])
    scheduler.add_job(send_reminders, 'cron', hour=20, minute=0, args=[updater.bot])
    scheduler.add_job(send_progress_report, 'cron', day='1,15', hour=12, minute=0, args=[updater.bot])
    scheduler.start()
